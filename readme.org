...
- [[#installation][Installation]]
  ...
  - [[#use-with-cpmcmake][Use with CPM.cmake]]
- [[#usage][Usage]]
...
This repository also provides an interface CMake target, =vcfpp::vcfpp=.
The target adds the include path for =vcfpp.h=, requires C++11, and links to
=htslib=.

After installing vcfpp with CMake, use it from another project with:

#+begin_src cmake
find_package(vcfpp REQUIRED)

add_executable(my_tool main.cpp)
target_link_libraries(my_tool PRIVATE vcfpp::vcfpp)
#+end_src

=htslib= must be installed and discoverable by CMake. If =pkg-config= can find
=htslib=, vcfpp will use that imported target. Otherwise CMake searches for
=htslib/hts.h= and =libhts= directly.

** Use with CPM.cmake

For projects using [[https://github.com/cpm-cmake/CPM.cmake][CPM.cmake]], add vcfpp as a package and link the exported
target:
...
CPMAddPackage(
    NAME vcfpp
    GITHUB_REPOSITORY Zilong-Li/vcfpp
    GIT_TAG v0.7.4
)
...
Your source can then include the header normally:

#+begin_src C++
#include <vcfpp.h>
#+end_src

Keep =htslib= installed on the build machine because =vcfpp.h= includes
=htslib= headers and links to =libhts=.
