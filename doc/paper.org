...
\begin{titlepage} \maketitle
  ...
  methods to manipulate it in different languages. Static languages
  like C++ has the superiority of performance as C, and its modern
  version strives to provide highly easy to use API for fast
  developing program. In this work, we present vcfpp, a C++ API of
  htslib, providing a rapid and intuitive interface to manipulate
  VCF/BCF files safely and as easily as writing python script. We
  demonstrate an example of how minimal vcfpp code can read/write
  variant records easily. Additionally, we demonstrate how vcfpp can
  be useful to Statisticians analyzing genetic variants in R. In the
  benchmarking, a simple R script using vcfpp API shows lower memory
  footprint and shorter runtime than scripts using vcfR and
  cyvcf2. Command line tools using vcfpp are available for
  performaning PBWT-based analysis using large reference panel.
\end{titlepage}
#+end_export
...
vcfpp implements four classes for manipulating VCF/BCF as showed in [[tb:class]]. 
...
#+caption: Performance of counting heterozygous genotypes per sample in the 1000 Genome Project for chromosome 21. (*) RAM used by sourceCpp function.
#+attr_latex: :align lrrrr :placement [H]
|------------------+----------+-------+-------------|
| API              | Time (s) | Ratio |    RAM (Gb) |
|------------------+----------+-------+-------------|
| vcfpp::BcfReader |      118 |   1.0 | 0.071+0.28* |
